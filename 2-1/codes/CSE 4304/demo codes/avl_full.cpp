...
void display(struct node* check){
    ...
    if(check!=NULL) cout<<check->data<<" ("<<check->b_fact<<")";
...
/**
replaces heavy child with imbalanced node and puts imbalanced in it's right tree
**/
void right_rotate(struct node* imb){
    cout<<"right_rotate("<<imb->left->data<<")"<<endl;
...
void left_rotate(struct node* imb){
    cout<<"left_rotate("<<imb->right->data<<")"<<endl;
...
void avl(struct node* t){
    ...
    while(bal!=NULL){
        ...
        if(bal->b_fact>1 && bal->left->data>t->data){
            display(root);
            cout<<endl;
            cout<<"imbalanced node: "<<bal->data<<endl;
            cout<<"LL case"<<endl;
            right_rotate(bal);
        }
...
int main(void){
    ...
    while(1){
        ...
        if(n==-1) break;
            ...
            //calls avl after each insertion;
            avl(t);
            tot_node++;
            cout<<endl;
            display(root);
